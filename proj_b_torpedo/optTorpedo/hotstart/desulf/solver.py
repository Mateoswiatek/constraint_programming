...
class TorpedoScheduler:
    ...
    def _create_initial_solution(self, randomize: bool = False):
        """Create initial solution using greedy assignment.

        Args:
            randomize: If True, add randomness to explore different starting points:
                       - Shuffle conversion order (instead of strictly by opening time)
                       - Pick from top-k feasible pourings randomly (instead of always best)
        """
        self.pouring_to_conv = {}
        self.conv_to_pouring = {}
        ...
        # Randomize: shuffle conversion order with some noise
        if randomize:
            # Add random noise to preserve some structure but explore different orderings
            # Shuffle in groups - divide into chunks and shuffle within/between
            chunk_size = max(5, len(conv_order) // 10)
            chunks = [conv_order[i:i+chunk_size] for i in range(0, len(conv_order), chunk_size)]
            for chunk in chunks:
                random.shuffle(chunk)
            random.shuffle(chunks)
            conv_order = [j for chunk in chunks for j in chunk]

        # For each conversion, find the best available pouring
        assigned_pourings = set()

        # How many top candidates to consider when randomizing
        TOP_K = 3
        ...
        for j in conv_order:
            ...
            if randomize:
                # Collect all feasible candidates with their slack
                candidates = []
                for i in self.feasible_pourings[j]:
                    if i in assigned_pourings:
                        continue
                    earliest_arrival = self.data.metal_starts[i] + self.data.min_travel_time(i, j)
                    slack = self.data.conv_opens[j] - earliest_arrival
                    if slack >= 0:
                        candidates.append((i, slack))

                if candidates:
                    # Sort by slack and pick randomly from top-k
                    candidates.sort(key=lambda x: x[1])
                    top_candidates = candidates[:TOP_K]
                    best_pouring = random.choice(top_candidates)[0]
            else:
                # Deterministic: always pick minimum slack
                for i in self.feasible_pourings[j]:
                    if i in assigned_pourings:
                        continue
    ...
    def _move_reduce_desulf(self) -> bool:
        ...
        Strategy: Find a pouring with high desulf time and swap it with another
        pouring that would have lower desulf for that conversion.
        """
        # Find pourings with significant desulf time
        high_desulf_pourings = []
        for i, j in self.pouring_to_conv.items():
            if j == -1:
                continue
            sulfur_reduction = max(0, self.data.metal_sulfur[i] - self.data.conv_max_sulfur[j])
            if sulfur_reduction > 0:
                desulf_time = sulfur_reduction * self.data.time_desulf_one
                high_desulf_pourings.append((i, j, desulf_time))

        if not high_desulf_pourings:
            return False

        # Sort by desulf time descending
        high_desulf_pourings.sort(key=lambda x: -x[2])

        # Try to improve the worst case
        # Pick from top 20% or at least one
        top_count = max(1, len(high_desulf_pourings) // 5)
        p1, j1, current_desulf = random.choice(high_desulf_pourings[:top_count])

        SLACK = 5
        best_swap = None
        best_improvement = 0

        # Look for swaps that reduce total desulf
        for p2, j2 in self.pouring_to_conv.items():
            if p2 == p1:
                continue

            if j2 == -1:
                # p2 is emergency - can it serve j1 with less desulf?
                if not self.data.can_reach(p2, j1, slack=SLACK):
                    continue

                # Calculate new desulf if p2 serves j1
                new_desulf_p2_j1 = max(0, self.data.metal_sulfur[p2] - self.data.conv_max_sulfur[j1]) * self.data.time_desulf_one

                # p1 would go to emergency (0 desulf counted)
                improvement = current_desulf - new_desulf_p2_j1

                if improvement > best_improvement:
                    best_improvement = improvement
                    best_swap = (p2, j2, 'emergency_swap')
            else:
                # p2 is serving j2 - can we swap?
                if not self.data.can_reach(p1, j2, slack=SLACK):
                    continue
                if not self.data.can_reach(p2, j1, slack=SLACK):
                    continue

                # Current desulf for both
                current_p1_j1 = max(0, self.data.metal_sulfur[p1] - self.data.conv_max_sulfur[j1]) * self.data.time_desulf_one
                current_p2_j2 = max(0, self.data.metal_sulfur[p2] - self.data.conv_max_sulfur[j2]) * self.data.time_desulf_one
                current_total = current_p1_j1 + current_p2_j2

                # New desulf if swapped
                new_p1_j2 = max(0, self.data.metal_sulfur[p1] - self.data.conv_max_sulfur[j2]) * self.data.time_desulf_one
                new_p2_j1 = max(0, self.data.metal_sulfur[p2] - self.data.conv_max_sulfur[j1]) * self.data.time_desulf_one
                new_total = new_p1_j2 + new_p2_j1

                improvement = current_total - new_total

                if improvement > best_improvement:
                    best_improvement = improvement
                    best_swap = (p2, j2, 'conversion_swap')

        if best_swap is None:
            # No improving swap found - try random swap for exploration
            candidates = []
            for p2, j2 in self.pouring_to_conv.items():
                if p2 == p1:
                    continue
                if j2 == -1:
                    if self.data.can_reach(p2, j1, slack=SLACK):
                        candidates.append((p2, j2, 'emergency_swap'))
                else:
                    if self.data.can_reach(p1, j2, slack=SLACK) and self.data.can_reach(p2, j1, slack=SLACK):
                        candidates.append((p2, j2, 'conversion_swap'))

            if not candidates:
                return False

            best_swap = random.choice(candidates)

        p2, j2, swap_type = best_swap

        if swap_type == 'emergency_swap':
            # p1 goes to emergency, p2 takes j1
    ...
    def solve(self, time_limit: float = 300.0, verbose: bool = True,
              ...
              random_restart: bool = False) -> dict:
        """
        ...
        Args:
            ...
            random_restart: Enable random restart after repeated reheat failures
        """
        ...
        # Track best metrics for random restart validation
        best_missed = initial_missed
        best_torpedoes = initial_concurrent
        ...
        initial_temperature = current_score / 10.0 if current_score > 0 else 1000000
        temperature = initial_temperature
        cooling_rate = 0.9997  # Slower cooling for better exploration
        min_temperature = 0.1  # Lower min temp for finer search
        ...
        # Aggressive reheat settings
        REHEAT_TIME_THRESHOLD = 15.0  # Reheat if no improvement for 15 seconds (faster)
        REHEAT_ITER_THRESHOLD = 5000  # Or after 5000 iterations without improvement

        # Adaptive reheat escalation - multipliers for temperature on consecutive reheats
        REHEAT_ESCALATION = [1.0, 2.0, 5.0, 10.0, 20.0]
        reheat_count = 0

        # Tabu list - remember recent swaps to avoid cycling
        TABU_SIZE = 500  # Increased from 100 for better diversity
        tabu_list = deque(maxlen=TABU_SIZE)

        # Perturbation settings - number of random swaps on reheat
        PERTURB_MIN = 15  # Increased from 5 for stronger perturbation
        PERTURB_MAX = 40  # Increased from 15 for stronger perturbation

        # Rotating reheat strategies
        REHEAT_STRATEGIES = ['random_swaps', 'focus_high_desulf', 'shuffle_emergencies']
        current_strategy_idx = 0

        # Random restart settings
        RANDOM_RESTART_THRESHOLD = 5  # After 5 consecutive reheats without improvement, do random restart
        reheats_since_improvement = 0

        # Move probabilities: swap, reassign_missed, unassign, chain, resolve_emergency, reduce_overlap, reduce_desulf
        # Added reduce_desulf with 20% probability, adjusted others
        move_probs = [0.15, 0.15, 0.05, 0.10, 0.10, 0.15, 0.30]  # 30% for desulf reduction
        ...
        while time.time() - start_time < time_limit:
            ...
            elif r < (cumsum := cumsum + move_probs[5]):
                move_made = self._move_reduce_overlap()
            else:
                move_made = self._move_reduce_desulf()
            ...
            # Create a hashable representation of current assignment for tabu check
            assignment_hash = tuple(sorted(self.pouring_to_conv.items()))
            is_tabu = assignment_hash in tabu_list

            # Accept move if: improves score, OR passes SA criterion AND not tabu (unless very good)
            accept_move = False
            if delta < 0:
                accept_move = True  # Always accept improvements
            elif not is_tabu and random.random() < math.exp(-delta / max(temperature, 1)):
                accept_move = True  # Accept non-tabu worse moves with SA probability

            if accept_move:
                current_score = new_score
                # Add to tabu list
                tabu_list.append(assignment_hash)

                if new_score < best_score:
                    best_score = new_score
                    best_assignment = self.pouring_to_conv.copy()
                    # Update best metrics for random restart validation
                    best_missed = self._count_missed_conversions()
                    best_torpedoes = self._count_max_concurrent_trips()
                    improvements += 1
                    no_improve_count = 0
                    last_improvement_time = time.time()
                    reheat_count = 0  # Reset reheat escalation on improvement
                    reheats_since_improvement = 0  # Reset random restart counter

                    if verbose and improvements % 10 == 0:
                        missed = best_missed
                        violations = self._count_deadline_violations()
                        max_conc = best_torpedoes
                        total_desulf = sum(c.desulf_time for c in self.cycles if c.conv_idx >= 0)
                        elapsed = time.time() - start_time
                        print(
                            f"[{elapsed:.1f}s] #{improvements}: {missed} missed, {violations} violations, {max_conc} torpedoes, {total_desulf} desulf"
                        )
                else:
                    no_improve_count += 1
            else:
            ...
            # Aggressive reheating based on time AND iterations without improvement
            current_time = time.time()
            time_since_improvement = current_time - last_improvement_time
            time_since_reheat = current_time - last_reheat_time

            should_reheat = False
            reheat_reason = ""

            # Time-based reheat: no improvement for REHEAT_TIME_THRESHOLD seconds
            if time_since_improvement > REHEAT_TIME_THRESHOLD and time_since_reheat > REHEAT_TIME_THRESHOLD:
                should_reheat = True
                reheat_reason = f"no improvement for {time_since_improvement:.1f}s"

            # Iteration-based reheat: too many iterations without improvement
            if no_improve_count > REHEAT_ITER_THRESHOLD:
                should_reheat = True
                reheat_reason = f"{no_improve_count} iterations without improvement"

            if should_reheat:
                # Adaptive reheat with escalation
                multiplier = REHEAT_ESCALATION[min(reheat_count, len(REHEAT_ESCALATION) - 1)]
                temperature = initial_temperature * multiplier
                reheat_count += 1
                no_improve_count = 0
                last_reheat_time = current_time

                # Select perturbation strategy (rotating)
                ...
                if strategy == 'random_swaps':
                    # Strategy 1: Random swaps between assigned and emergency pourings
                    for _ in range(perturb_count * 3):
                        if perturbations_done >= perturb_count:
                            break
                        assigned_pourings = [i for i, j in self.pouring_to_conv.items() if j >= 0]
                        emergency_pourings = [i for i, j in self.pouring_to_conv.items() if j == -1]
                        if assigned_pourings and emergency_pourings:
                            p1 = random.choice(assigned_pourings)
                            p2 = random.choice(emergency_pourings)
                            conv = self.pouring_to_conv[p1]
                            if self.data.can_reach(p2, conv, slack=0):
                                self.pouring_to_conv[p1] = -1
                                self.pouring_to_conv[p2] = conv
                                self.conv_to_pouring[conv] = p2
                                perturbations_done += 1

                elif strategy == 'focus_high_desulf':
                    # Strategy 2: Focus on pourings with highest desulfurization time
                    # Find pourings sorted by desulf time (descending)
                    high_desulf = []
                    for i, j in self.pouring_to_conv.items():
                        if j >= 0:
                            sulfur_red = max(0, self.data.metal_sulfur[i] - self.data.conv_max_sulfur[j])
                            if sulfur_red > 0:
                                high_desulf.append((i, j, sulfur_red * self.data.time_desulf_one))
                    high_desulf.sort(key=lambda x: -x[2])  # Sort by desulf time descending

                    emergency_pourings = [i for i, j in self.pouring_to_conv.items() if j == -1]
                    for p1, conv, _ in high_desulf[:perturb_count * 2]:
                        if perturbations_done >= perturb_count:
                            break
                        if not emergency_pourings:
                            break
                        # Try to swap with an emergency pouring
                        random.shuffle(emergency_pourings)
                        for p2 in emergency_pourings:
                            if self.data.can_reach(p2, conv, slack=0):
                                self.pouring_to_conv[p1] = -1
                                self.pouring_to_conv[p2] = conv
                                self.conv_to_pouring[conv] = p2
                                emergency_pourings.remove(p2)
                                emergency_pourings.append(p1)  # p1 is now emergency
                                perturbations_done += 1
                                break

                elif strategy == 'shuffle_emergencies':
                    # Strategy 3: Shuffle which pourings go to emergency
                    # Reassign some conversions to different pourings
                    assigned = [(i, j) for i, j in self.pouring_to_conv.items() if j >= 0]
                    emergency = [i for i, j in self.pouring_to_conv.items() if j == -1]

                    if assigned and emergency:
                        # Randomly select some assigned to become emergency and vice versa
                        random.shuffle(assigned)
                        random.shuffle(emergency)

                        for _ in range(min(perturb_count, len(assigned), len(emergency))):
                            if perturbations_done >= perturb_count:
                                break
                            p1, conv = assigned[perturbations_done % len(assigned)]
                            p2 = emergency[perturbations_done % len(emergency)]

                            # Check if p2 can take the conversion
                            if self.data.can_reach(p2, conv, slack=0):
                                self.pouring_to_conv[p1] = -1
                                self.pouring_to_conv[p2] = conv
                                self.conv_to_pouring[conv] = p2
                ...
                if perturbations_done > 0:
                    self._rebuild_cycles_with_scheduling()
                    current_score = self._evaluate_solution()

                # Track reheats for random restart
                reheats_since_improvement += 1

                if verbose:
                    elapsed = current_time - start_time
                    print(f"[{elapsed:.1f}s] ADAPTIVE REHEAT #{reheat_count} [{strategy}] ({reheat_reason}) -> temp={temperature:.2e} (x{multiplier}), {perturbations_done} perturbations")

                # Random restart: after too many reheats without improvement, start fresh
                if random_restart and reheats_since_improvement >= RANDOM_RESTART_THRESHOLD:
                    if verbose:
                        elapsed = current_time - start_time
                        print(f"[{elapsed:.1f}s] RANDOM RESTART attempt (no improvement after {reheats_since_improvement} reheats)")

                    # Save current state in case we need to reject the restart
                    saved_p2c = self.pouring_to_conv.copy()
                    saved_c2p = self.conv_to_pouring.copy()
                    saved_score = current_score

                    # Create a completely new RANDOMIZED solution to explore different region
                    self._create_initial_solution(randomize=True)
                    self._rebuild_cycles_with_scheduling()

                    # Check if new solution is acceptable (not worse in missed/torpedoes)
                    new_missed = self._count_missed_conversions()
                    new_torpedoes = self._count_max_concurrent_trips()

                    # Accept only if: missed <= best_missed AND torpedoes <= best_torpedoes
                    if new_missed > best_missed or new_torpedoes > best_torpedoes:
                        # Reject restart - new solution is worse, restore previous state
                        if verbose:
                            print(f"         REJECTED: {new_missed} missed (best: {best_missed}), {new_torpedoes} torpedoes (best: {best_torpedoes})")
                        self.pouring_to_conv = saved_p2c
                        self.conv_to_pouring = saved_c2p
                        self._rebuild_cycles_with_scheduling()
                        # Don't reset counters - keep trying
                        reheats_since_improvement = 0  # But reset to avoid infinite loop
                    else:
                        # Accept restart - exploring new region of solution space
                        current_score = self._evaluate_solution()

                        # Reset counters but keep best solution
                        reheats_since_improvement = 0
                        reheat_count = 0
                        current_strategy_idx = 0
                        tabu_list.clear()
                        ...
                        if verbose:
                            violations = self._count_deadline_violations()
                            total_desulf = sum(c.desulf_time for c in self.cycles if c.conv_idx >= 0)
                            print(f"         ACCEPTED: {new_missed} missed, {violations} violations, {new_torpedoes} torpedoes, {total_desulf} desulf")
                            print(f"         >>> Exploring new region of solution space (desulf: {total_desulf}) <<<")
...
"""
    ...
    parser.add_argument('--random-restart', dest='random_restart', action='store_true',
                        help='Enable random restart after repeated failures (default: disabled)')
    ...
    print(f"Random restart: {'ENABLED' if args.random_restart else 'DISABLED'}")
