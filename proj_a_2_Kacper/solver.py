#!/usr/bin/env python3
"""
Enrollment solver using OR-Tools CP-SAT.
Reads a MiniZinc .dzn instance, builds a CP model replicating the problem
statement from README.md, and outputs an assignment along with objective parts.
"""
...
class Instance:
    ...
    student_prefers: List[List[int]]  # shape (student, group)
...
def parse_dzn(path: Path) -> Instance:
    ...
    if len(student_break_importance) != n_students:
        raise ValueError("student_break_importance length mismatch")
    if len(group_class) != n_groups:
        raise ValueError("group_class length mismatch")
...
def build_model(inst: Instance) -> Tuple[cp_model.CpModel, Dict]:
    ...
    for s in range(n_students):
        ...
        # "this value in theory could go below zero, but we should assume the 0 is the minimal value"
...
def solve_instance(inst: Instance, time_limit: float, workers: int) -> Dict:
    model, aux = build_model(inst)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Solve enrollment instances with CP-SAT"
    )
    parser.add_argument("instance", type=Path, help="Path to .dzn instance")
    parser.add_argument(
        "--time-limit", type=float, default=30.0, help="Solver time limit in seconds"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=0,
        help="Number of parallel search workers (0=auto)",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Optional path to save the solution (defaults to competition.sol when instance name contains 'competition')",
    )
    args = parser.parse_args()

    inst = parse_dzn(args.instance)
    result = solve_instance(inst, time_limit=args.time_limit, workers=args.workers)
